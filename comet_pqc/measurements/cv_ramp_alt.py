...
class CVRampAltMeasurement(MatrixMeasurement, LCRMixin, EnvironmentMixin, AnalysisMixin):
    ...
    def quick_ramp_zero(self, lcr):
        ...
        self.update_state({"lcr_output": lcr_output})
        if lcr_output:
            ...
            for step, voltage in enumerate(ramp):
                ...
                self.update_state({"lcr_voltage": voltage})
                time.sleep(waiting_time_after)
        self.update_state({"lcr_output": self.lcr_get_bias_state(lcr)})
        self.set_message("")
    ...
    def measure(self, lcr):
        ...
        for voltage in ramp:
            with benchmark_step:
                ...
                dt = time.time() - t0
                est.advance()
                self.set_message("{} | V Source {}".format(format_estimate(est), format_metric(voltage, "V")))
                self.set_progress(*est.progress)
    ...
    def finalize(self, lcr):
        ...
        self.update_state({"lcr_current": None})
