...
class TableTab(PreferencesTab):
    ...
    def on_remove_step_clicked(self):
        item = self._steps_tree.current
        if item:
            if showQuestion(f"Do you want to remove step size {item[0].value!r}?"):
    ...
    def store(self):
        ...
        settings.set_table_z_limit(self._z_limit_movement_number.value)
        # Probecard limits
        ...
        settings.set_retry_contact_overdrive(self._recontact_overdrive_number.value)
